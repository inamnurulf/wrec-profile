...
export default function FileDetailPage() {
  ...
  const t = useTranslations("PublicFileDetail");
      ...
      <div className="relative">
        ...
        <main className="max-w-7xl mx-auto px-4 mt-6 grid grid-cols-1 md:grid-cols-12 gap-6">
          <div className="md:col-span-8 bg-white rounded-2xl shadow-sm ring-1 ring-slate-200 p-6 my-3">
            ...
            <h2 className="mb-2 text-sm font-semibold text-slate-600">{t("links")}</h2>
            <div className="space-y-2 mb-6">
              {[1, 2, 3].map((i) => (
                <div key={i} className="flex items-center justify-between gap-3 rounded-xl border border-slate-200 p-3">
            ...
            <h2 className="mb-2 text-sm font-semibold text-slate-600">{t("metadata")}</h2>
      ...
      <header className="border-b bg-gradient-to-br from-emerald-50 via-white to-slate-50">
        <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3 text-gray-600">
            ...
            <Chip>
              {t("created")} {fmtDate(file.created_at) || "—"}
            </Chip>
            <Chip>
              {t("updated")} {fmtDate(file.updated_at) || "—"}
            </Chip>
            <Chip tone={statusTone}>
              {file.is_published ? t("status.published") : t("status.draft")}
            </Chip>
          </div>
        </div>
      </header>
        ...
        <aside className="md:col-span-4 space-y-6 md:sticky md:top-20 self-start">
          <div className="bg-white rounded-2xl shadow-sm ring-1 ring-slate-200 p-5">
            ...
            <ul className="text-sm text-gray-600 space-y-1">
              <li>
                <span className="text-gray-500">{t("slug")}:</span> {file.slug}
              </li>
              <li>
                <span className="text-gray-500">{t("id")}:</span> {file.id}
              </li>
